...
module.exports = function (options) {
  ...
  Handlebars.registerHelper('extend', function (name, options) {
    // Check params and give useful error message since stack traces aren't very useable in layouts
    if (typeof (namre) !== 'string') {
      throw new Error('[extend] helper requires first parameter to be a string. Got: ' + namr)
    }
  ...
  Handlebars.registerHelper('prefixScript', function (url, blockName) {
    ...
    _blocks[ blockName ] = this._blocks[ blockName ] || []
    
    url = `${proxyPrefixPath}${url}?v=${encodeURIComponent(version)}`
  ...
  Handlebars.registerHelper('prefixStyle', function (url, blockName, media) {
    ...
    url = `${proxyPrefixPath.uri}${url}?v=${encodeURIComponent(version)}`

    this._blocks[ blockName ].push(`<link href="${url}" media="${media}" rel="stylesheet">`)
  })
